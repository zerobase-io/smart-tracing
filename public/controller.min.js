...
$("#modal-register-notice").modal("show"),$("body").on("click","#register-notice-agree",function(a){$("#register-notice-agree").addClass("btn-loading");$("#modal-scan-notice").modal("show");router.fetchIP({sdvid:m},function(a){router.fingerprint(a,function(a){router.create(a,function(a){router.scan(a,function(a){router.init()})})})})})));$("body").on("click","a[data-toggle='view']",function(a){a.preventDefault();window.scrollTo(0,0);$('a[href="'+$(a.currentTarget).attr("href")+'"]').tab("show")});
$("body").on("click","#landing-about",function(a){console.log("hello");$('a[href="#tabs-about"]').tab("show")});$("body").on("click","#landing-generate",function(a){localStorage.getItem("dvid")?$('a[href="#page-code"]').tab("show"):($("#modal-register-notice").attr("data-redirect","true"),$("#modal-register-notice").modal("show"))});$("body").on("click","#show-registration",function(a){$("#modal-register-notice").modal("show")});$("body").on("click","#nav-code",function(a){$('a[href="#page-code"]').tab("show");
$(".notice-code").addClass("active");$(".notice-code .dimmer-notice").show()});$("body").on("click","#nav-main",function(a){$('a[href="#page-main"]').tab("show");$(".notice-code").addClass("active");$(".notice-code .dimmer-notice").show()});$("body").on("click","#reveal-code",function(a){console.log("Reveal");$(".notice-code").removeClass("active");$(".notice-code .dimmer-notice").hide()});$("body").on("click",".reveal-code-alt",function(a){$("#qrcode-alt").empty();$("#qrcode-alt").append('<img class="qr-logo-overlay" src="/dist/img/qr_logo.png">');
$(".notice-code").addClass("active");$(".notice-code .dimmer-notice").show();$(this).attr("data-id")?(router.genQR($(this).attr("data-id"),"qrcode-alt"),$("#title-code-alt").text($(this).find(".title-code-alt").text()),$('a[href="#page-code-alt"]').tab("show")):console.log("error displaying alt")});$("body").on("click","#generate-alt-code",function(a){$("#generate-alt-code").addClass("btn-loading-dark");router.fetchIP({},function(a){router.fingerprint(a,function(a){router.createAlt(a)})})});$("body").on("click",
"#nav-scan",function(a){window.sound=new Howl({src:["/assets/audio/beep.mp3"]});$('a[href="#page-scan"]').tab("show");navigator.mediaDevices.getUserMedia({video:{facingMode:"environment"}}).then(function(a){k.srcObject=a;k.setAttribute("playsinline",!0);k.play();h||(h=window.requestAnimationFrame(c));$("body").on("click","#nav-main-x",function(b){$('a[href="#page-main"]').tab("show");a.getTracks().forEach(function(a){a.stop()});h&&(window.cancelAnimationFrame(h),h=void 0)})})});var k=document.createElement("video"),
l=document.getElementById("canvas"),g=l.getContext("2d");document.getElementById("loadingMessage");document.getElementById("output");document.getElementById("outputMessage");document.getElementById("outputData");var h});