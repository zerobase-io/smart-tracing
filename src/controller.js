...
const defaultRequestOpts = { // TODO: simplify requests
	cache: false,
	dataType: 'json',
...
const controller = (() => ({
	create: ({ fingerprint }, callback) => { // TODO: Re-name to something more explicit -> createDevice?
		...
		const req = {
			...
			success: responseData => {
				...
				console.log('create -> data-redirect value:', $('#modal-register-notice').attr('data-redirect'));
	...
	submit_organization: inputs => {
		...
		const postData = { // TODO: clean up data so we don't have to do this
	...
	submit_site: inputs => {
		...
		const req = {
			...
			success: responseData => {
				...
				$(inputs.modal_id).find('.alert-success a').attr('href', link).attr("target", "_blank");
				$(inputs.modal_id).find('.alert-success').removeClass('d-none');
				$(inputs.button_id).replaceWith(`<a href="${link}", target="_blank", class="btn btn-secondary btn-block">Check your email</a>`);
			},
			error: e => {
