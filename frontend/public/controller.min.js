...
$("#scan-notice-error").removeClass("d-none"));console.log("Scan (Callback):",c);console.log("Scan (Data):",a);c&&c()},error:function(a){console.log(a);console.log("Scan: Error");$("#scan-notice-loading").addClass("d-none");$("#scan-notice-error").removeClass("d-none");console.log("Scan Error (Callback):",c);c&&c()}})},fetchNews:function(){jQuery.ajax({url:"https://newsapi.org/v2/top-headlines?q=coronavirus&country=de&apiKey=84ca05784bfb4228b4dc2396fd09d950",cache:!1,contentType:!1,processData:!1,
type:"GET",success:function(a){"ok"===a.status&&a.articles&&(console.log("Fetched News:"),console.log(a),a.articles.map(function(a){var b=$("#template-news").clone();b.find(".news-href").attr("href",a.url);b.find(".news-source").text(a.source.name);b.find(".news-title").text(a.title);b.find(".news-published").text(a.publishedAt);b.find(".news-image").css("background-image","url("+a.urlToImage+")");b.removeAttr("id");b.removeClass("d-none");$("#news-feed-container").append(b)}))},error:function(a){console.log(a)}})},
fetchIP:function(a,c){jQuery.ajax({url:"https://api.ipify.org?format=json",cache:!1,contentType:!1,processData:!1,type:"GET",success:function(b){console.log("IP (Success):",b);console.log("IP (Callback):",c);console.log("IP (Data):",b);if(c&&a)a.ip=b.ip,c(a);else return b.ip},error:function(b){console.log(b);if(c&&a)a.ip=ipObj.ip,c(a);else return b}})},fingerprint:function(a,c){var b=function(){var b=new Date;Fingerprint2.get(function(d){d=Fingerprint2.x64hash128(d.map(function(a){return a.value}).join(),
31);console.log("Fingerprint (Time):",(new Date).valueOf()-b.valueOf());console.log("Fingerprint (Generated):",d);if(c&&a)console.log("Fingerprint >1 (Callback):",c),console.log("Fingerprint >1 (Data):",a),a.fingerprint=d,c(a);else return d})};window.requestIdleCallback?(console.log("Fingerprint: requestIdleCallback"),requestIdleCallback(b)):(console.log("Fingerprint: setTimeout"),setTimeout(b,500))},init:function(){console.log("Initialized");if(750>window.innerWidth){$("#mobile-functions").removeClass("d-none");
$("#desktop-notice").addClass("d-none");var a=localStorage.getItem("dvid"),c=localStorage.getItem("dvid_alt")?JSON.parse(localStorage.getItem("dvid_alt")).length:0;a?(console.log("Initialized with Device ID."),1<$("#qrcode").children().length?console.log("QR already generated"):router.genQR(a,"qrcode"),0<c&&(console.log("alts available:",c),router.displayAlt(),$("#alternative-code-card").removeClass("d-none"),$("#notice-alt-code").addClass("d-none"))):(console.log("no registered dvid"),$("#mobile-functions").addClass("d-none"),
$("#registration-notice").removeClass("d-none"),$("#modal-register-notice").modal("show"),$("body").on("click","#register-notice-agree",function(a){$("#register-notice-agree").addClass("btn-loading");router.fetchIP({},function(a){router.fingerprint(a,function(a){console.log("cancel-route: register -> create:",a);router.create(a)})})}))}}}}();
...
slidesToShow:3,slidesToScroll:3});[].slice.call(document.querySelectorAll("[data-mask]")).map(function(a){console.log("maskEl",a);return new IMask(a,{mask:a.dataset.mask,lazy:"true"===a.dataset["mask-visible"]})});var b=document.createElement("a");b.href=window.location.href;var d=b.protocol,e=b.hostname,f=b.pathname;b=f.split("/")[1];var m=f.split("/")[2];f=localStorage.getItem("dvid");"http:"==d&&"localhost"==e&&"s"==b&&m?(console.log("Scan Access Point"),window.history.replaceState({},"Home","/"),
f?(console.log("Has registered ID on scan"),$("#modal-scan-notice").modal("show"),router.fetchIP({sdvid:m},function(a){router.fingerprint(a,function(a){router.scan(a,function(){router.init()})})})):(console.log("Does not have registed ID on scan"),750>window.innerWidth&&$("#desktop-notice").addClass("d-none"),$("#mobile-functions").addClass("d-none"),$("#registration-notice").removeClass("d-none"),$("#modal-register-notice").modal("show"),$("body").on("click","#register-notice-agree",function(a){$("#register-notice-agree").addClass("btn-loading");
$("#modal-scan-notice").modal("show");router.fetchIP({sdvid:m},function(a){router.fingerprint(a,function(a){router.create(a,function(a){router.scan(a,function(){router.init()})})})})}))):("s"==b?(console.log("Invalid Access Point"),window.history.replaceState({},"Home","/")):console.log("No Conditions Met"),router.init());$("body").on("click","#landing-about",function(a){console.log("hello");$('a[href="#tabs-about"]').tab("show")});$("body").on("click","#landing-generate",function(a){localStorage.getItem("dvid")?
...
$(".notice-code .dimmer-notice").show()});$("body").on("click","#reveal-code",function(a){console.log("Reveal");$(".notice-code").removeClass("active");$(".notice-code .dimmer-notice").hide()});$("body").on("click",".reveal-code-alt",function(a){$("#qrcode-alt").empty();$("#qrcode-alt").append('<img class="qr-logo-overlay" src="/assets/img/qr_logo.png">');$(".notice-code").addClass("active");$(".notice-code .dimmer-notice").show();$(this).attr("data-id")?(router.genQR($(this).attr("data-id"),"qrcode-alt"),
$("#title-code-alt").text($(this).find(".title-code-alt").text()),$('a[href="#page-code-alt"]').tab("show")):console.log("error displaying alt")});$("body").on("click","#generate-alt-code",function(a){$("#generate-alt-code").addClass("btn-loading-dark");router.fetchIP({},function(a){router.fingerprint(a,function(a){router.createAlt(a)})})});$("body").on("click","#nav-scan",function(a){window.sound=new Howl({src:["/assets/audio/beep.mp3"]});$('a[href="#page-scan"]').tab("show");navigator.mediaDevices.getUserMedia({video:{facingMode:"environment"}}).then(function(a){k.srcObject=
a;k.setAttribute("playsinline",!0);k.play();h||(h=window.requestAnimationFrame(c));$("body").on("click","#nav-main-x",function(b){$('a[href="#page-main"]').tab("show");a.getTracks().forEach(function(a){a.stop()});h&&(window.cancelAnimationFrame(h),h=void 0)})})});var k=document.createElement("video"),l=document.getElementById("canvas"),g=l.getContext("2d"),h});