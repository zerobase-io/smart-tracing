...
var router = (function () {
    return {
        ...
        displayAlt: function () {
            JSON.parse(localStorage.getItem("dvid_alt")).map((alt, index) => {
                var template = $("#template-code-alt").clone();
                template
                    .find(".title-code-alt")
        ...
        fetchNews: function () {
            var opts = {
                url: "https://newsapi.org/v2/top-headlines?q=coronavirus&" +
                    "country=de&" +
        ...
        fingerprint: function (data, cb) {
            var fingerprintReport = function () {
                ...
                Fingerprint2.get(function (components) {
                    var murmur = Fingerprint2.x64hash128(components
                        .map(pair => {
                        return pair.value;
                    })
                        .join(), 31);
...
$(function () {
    ...
    var protocol = scan_parser.protocol, hostname = scan_parser.hostname, pathname = scan_parser.pathname, action = pathname.split("/")[1], sdvid = pathname.split("/")[2];
    var dvid = localStorage.getItem("dvid");
    if (protocol == "http:" &&
        hostname == "localhost" &&
        action == "s" &&
    ...
    function scanLoop() {
        ...
        if (video.readyState === video.HAVE_ENOUGH_DATA) {
            ...
            if (code) {
                ...
                var protocol = parser.protocol, hostname = parser.hostname, pathname = parser.pathname, action = pathname.split("/")[1], sdvid = pathname.split("/")[2];
